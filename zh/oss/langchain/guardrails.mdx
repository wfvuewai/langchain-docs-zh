---
title: 防护措施
description: 为你的代理实现安全检查与内容过滤
---

防护措施通过在你的代理执行的关键节点验证和过滤内容，帮助你构建安全、合规的 AI 应用。它们可以检测敏感信息、执行内容策略、验证输出，并在问题发生前防止不安全的行为。
...
你可以使用[中间件](/oss/langchain/middleware)在关键位置拦截执行来实现防护措施——在代理启动前、完成后，或在模型和工具调用前后。
...
<CardGroup cols={2}>
    <Card title="确定性防护措施" icon="list-check">
        使用基于规则的逻辑，如正则表达式模式、关键词匹配或显式检查。快速、可预测且成本低，但可能遗漏细微的违规。
    </Card>
    <Card title="基于模型的防护措施" icon="brain">
        使用 LLM 或分类器通过语义理解来评估内容。捕获规则遗漏的微妙问题，但速度较慢且成本更高。
    </Card>
</CardGroup>

LangChain 既提供内置的防护措施（例如 [PII 检测](#pii-detection)、[人工介入](#human-in-the-loop)），也提供灵活的中间件系统，用于使用任一方法构建自定义防护措施。

## 内置防护措施

### PII 检测

LangChain 提供了用于检测和处理对话中个人身份信息（PII）的内置中间件。该中间件可以检测常见的 PII 类型，如电子邮件、信用卡、IP 地址等。

PII 检测中间件适用于以下场景：医疗保健和金融应用中需要满足合规要求、需要清理日志的客户服务代理，以及一般处理敏感用户数据的任何应用。
...
LangChain 提供了在执行敏感操作前需要人工审批的内置中间件。这是高风险决策中最有效的防护措施之一。

人工介入中间件适用于以下场景：金融交易和转账、删除或修改生产数据、向外部方发送通信，以及任何具有重大业务影响的操作。
...
对于更复杂的防护措施，你可以创建在代理执行前后运行的自定义中间件。这让你完全控制验证逻辑、内容过滤和安全检查。

### 代理前防护措施

使用"代理前"钩子在每次调用的开始时验证请求。这对于会话级别的检查（如身份验证、速率限制或在任何处理开始前阻止不当请求）非常有用。
...
使用"代理后"钩子在返回给用户前验证最终输出。这对于基于模型的安全性检查、质量验证或对完整代理响应进行最终合规扫描非常有用。
...
你可以通过将多个防护措施添加到中间件数组中来堆叠它们。按顺序执行，让你能够构建分层保护：
