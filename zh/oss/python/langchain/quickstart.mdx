...
本快速开始教程向你展示如何在短短几分钟内创建一个功能齐全的 AI 智能体。

<Tip>
    **使用 AI 编码助手？**

    - 安装 [LangChain Docs MCP server](/use-these-docs) 让你的智能体能够获取最新的 LangChain 文档和示例。
    - 安装 [LangChain Skills](https://github.com/langchain-ai/langchain-skills) 以提升你的智能体在 LangChain 生态任务上的表现。
</Tip>
...
从[任何受支持的模型提供商](/oss/integrations/providers/overview)获取 API 密钥（例如 Google Gemini 或 OpenAI）。
...
```
  ...
  <Tab title="其他">
    查看完整的[聊天模型集成](/oss/integrations/chat)列表。
  </Tab>
</Tabs>

## 构建基础智能体

首先创建一个能够回答问题并调用工具的简单智能体。本示例中的智能体使用所选的语言模型、一个基础的天气函数作为工具，以及一个简单的提示词来引导其行为：
...
<Tip>
你可以通过更改模型名称并设置相应的 API 密钥来使用[任何受支持的模型](/oss/integrations/providers/overview)。使用 [LangSmith](https://smith.langchain.com) 追踪智能体内部发生的情况——按照[追踪快速开始](/langsmith/trace-with-langchain)进行设置。
</Tip>

## 构建真实世界的智能体

在以下示例中，你将构建一个能够回答关于文本文件问题的研究智能体。
在此过程中，你将探索以下概念：
...
<Steps>
    ...
    <Step title="创建工具">
        ...
        <Tip>
            工具应该有完善的文档：它们的名称、描述和参数名称都会成为模型提示词的一部分。
            LangChain 的 @[`@tool` decorator][@tool] 添加元数据并支持通过 `ToolRuntime` 参数进行运行时注入。
        ...
        <Note>
            ...
            或者，你也可以将 `schema` 属性定义为一个 [JSON schema](https://json-schema.org/overview/what-is-jsonschema) 对象。请注意，JSON schema **不会**在运行时进行验证。
    ...
    <Step title="配置模型">
        使用适合你用例的参数设置[语言模型](/oss/langchain/models)。例如：
    ...
    <Step title="添加记忆">
        ...
        为智能体添加[记忆](/oss/langchain/short-term-memory)，以在多次交互之间维持状态。这使得
        智能体能够记住之前的对话和上下文。
        ...
        <Info>
            在生产环境中，请使用将消息历史保存到数据库的持久化检查指针。
            参见[添加和管理记忆](/oss/langgraph/add-memory#manage-short-term-memory)了解更多详情。
        </Info>
    ...
    <Step title="创建并运行智能体">
        ...
        两者的主要区别在于，Deep Agents 自带一系列常用功能，如规划、文件系统工具和子智能体。

        当你需要以最少设置获得最大能力时，使用 Deep Agents；当你需要细粒度控制时，选择 LangChain 智能体。

        <Warning>
            由于代码会使用《了不起的盖茨比》的完整文本调用模型，因此会消耗大量 token。
    ...
    <Step title="查看结果">
        ...
        **How counts were computed:**
        ...
        如果你查看两个标签页的输出，会发现 LangChain 智能体提供了答案，但只是估算。该智能体缺乏回答此问题的工具。你也可能会遇到提示词过长的错误。

        另一方面，Deep Agent 能够：

        1. **规划其方法**，使用内置的 [`write_todos`](/oss/deepagents/harness#planning-capabilities) 工具分解研究任务。
        1. **加载文件**，通过调用 `fetch_text_from_url` 工具收集信息。
        1. **管理上下文**，使用文件系统工具（[`grep`](/oss/deepagents/harness#virtual-filesystem-access) 和 [`read_file`](/oss/deepagents/harness#virtual-filesystem-access)）。
        1. **生成子智能体**，根据需要将复杂的子任务委派给专门的子智能体。

        对于 LangChain 智能体，你需要实现更多能力才能达到类似的服务水平，并且可以根据需要沿途自定义。
...
你用 LangChain 构建的大多数有趣应用都会进行多次 LLM 调用。随着这些应用变得越来越复杂，能够检查智能体内部究竟发生了什么变得至关重要。最好的方法是使用 [LangSmith](https://smith.langchain.com)。
...
设置完成后，再次运行脚本，然后在 [LangSmith](https://smith.langchain.com) 上检查智能体调用期间发生的情况。
...
- **LangChain agents**：[添加和管理记忆](/oss/langgraph/add-memory#manage-short-term-memory)，[部署到生产环境](/oss/langgraph/deploy)
- **Deep Agents**：[自定义选项](/oss/deepagents/customization)，[持久化记忆](/oss/deepagents/long-term-memory)，[部署到生产环境](/oss/langgraph/deploy)