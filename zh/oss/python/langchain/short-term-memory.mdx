...
记忆是一个记录先前交互信息的系统。对于 AI 代理来说，记忆至关重要，因为它让代理能够记住之前的交互、从反馈中学习，并适应用户偏好。随着代理要处理包含大量用户交互的复杂任务，这一能力对于效率和用户满意度都变得不可或缺。

短期记忆让你的应用能够记住单个线程或对话中的先前交互。

<Note>
    线程将同一会话中的多次交互组织在一起，类似于电子邮件将消息组织在单个对话中。
</Note>

对话历史记录是最常见的短期记忆形式。长对话对当今的 LLM 构成挑战；完整的历史记录可能无法放入 LLM 的上下文窗口，导致上下文丢失或错误。

即使你的模型支持完整的上下文长度，大多数 LLM 在长上下文上的表现仍然不佳。它们会被陈旧或无关的内容"分散注意力"，同时还会面临更慢的响应时间和更高的成本。

聊天模型使用[消息](/oss/langchain/messages)来接受上下文，其中包括指令（系统消息）和输入（人类消息）。在聊天应用中，消息在人类输入和模型响应之间交替，导致消息列表随时间越来越长。由于上下文窗口有限，许多应用可以通过使用技术来移除或"遗忘"陈旧信息而受益。

<Tip>
    需要跨对话记住信息吗？使用[长期记忆](/oss/langchain/long-term-memory)来跨不同线程和会话存储和回忆用户特定或应用级别的数据。
</Tip>
...
<Info>
    ...
    通过将这些信息存储在图的状态中，代理可以访问给定对话的完整上下文，同时保持不同线程之间的分离。

    状态使用检查点器持久化到数据库（或内存中），以便线程可以在任何时候恢复。

    短期内存在代理被调用或某个步骤（如工具调用）完成时更新，并且在每个步骤开始时读取状态。
</Info>
...
<Note>
    有关更多检查点器选项，包括 SQLite、Postgres 和 Azure Cosmos DB，请参阅持久化文档中的[检查点器库列表](/oss/langgraph/persistence#checkpointer-libraries)。
</Note>

## 自定义代理记忆

:::python
默认情况下，代理使用 @[`AgentState`] 来管理短期记忆，特别是通过 `messages` 键管理的对话历史记录。

你可以扩展 @[`AgentState`] 以添加额外字段。自定义状态模式通过 @[`state_schema`] 参数传递给 @[`create_agent`]。
...
:::js
你可以通过创建带有状态模式的自定义中间件来扩展代理状态。自定义状态模式可以通过中间件中的 `stateSchema` 参数传递。建议优先使用 `StateSchema` 类来定义状态（普通的 Zod 对象也支持）。
...
启用[短期记忆](#usage)后，长对话可能会超出 LLM 的上下文窗口。常见的解决方案如下：
...
:::python
一种决定何时截断消息的方法是计算消息历史记录中的令牌数，并在接近限制时进行截断。如果你使用 LangChain，你可以使用修剪消息工具，指定要从列表中保留的令牌数量，以及用于处理边界的 `strategy`（例如，保留最后 `max_tokens` 个）。
:::
:::js
一种决定何时截断消息的方法是计算消息历史记录中的令牌数，并在接近限制时进行截断。如果你使用 LangChain，你可以使用修剪消息工具，指定要从列表中保留的令牌数量，以及用于处理边界的 `strategy`（例如，保留最后 `maxTokens` 个）。
:::

:::python
要在代理中修剪消息历史记录，使用 @[`@before_model`] 中间件装饰器：
...
:::js
要在代理中修剪消息历史记录，使用带有 `beforeModel` 钩子的 @[`createMiddleware`]：
...
你可以从图状态中删除消息来管理消息历史记录。

当你想要移除特定消息或清空整个消息历史记录时，这非常有用。

:::python
要从图状态中删除消息，你可以使用 `RemoveMessage`。

要使 `RemoveMessage` 生效，你需要使用带有 @[`add_messages`] [reducer](/oss/langgraph/graph-api#reducers) 的状态键。
...
:::js
要从图状态中删除消息，你可以使用 `RemoveMessage`。要使 `RemoveMessage` 生效，你需要使用带有 @[`messagesStateReducer`][messagesStateReducer] [reducer](/oss/langgraph/graph-api#reducers) 的状态键，例如 `MessagesValue`。
...
<Warning>
    删除消息时，**请确保**生成的消息历史记录是有效的。请检查你使用的 LLM 提供商的限制。例如：
...
如上所示，修剪或删除消息的问题在于，你可能会因裁剪消息队列而丢失信息。因此，一些应用受益于使用聊天模型总结消息历史记录这一更复杂的方法。

![Summary](/oss/images/summary.png)


:::python
要在代理中总结消息历史记录，使用内置的 [``SummarizationMiddleware``](/oss/langchain/middleware#summarization)：
...
请参阅 [`SummarizationMiddleware`](/oss/langchain/middleware#summarization) 了解更多配置选项。
:::
:::js
要在代理中总结消息历史记录，使用内置的 [`summarizationMiddleware`](/oss/langchain/middleware#summarization)：
...
在中间件中访问短期记忆（状态），基于对话历史记录或自定义状态字段创建动态提示。
