...
结构化输出允许代理以特定、可预测的格式返回数据。无需解析自然语言响应，你将获得以 JSON 对象、[Pydantic 模型](https://docs.pydantic.dev/latest/concepts/models/#basic-model-usage)或 dataclass 形式的结构化数据，可供你的应用直接使用。

<Tip>
本页介绍使用 `create_agent` 的代理结构化输出。要直接在模型上使用结构化输出（不使用代理），请参阅[模型 - 结构化输出](/oss/langchain/models#structured-output)。
</Tip>
...
:::
:::js
结构化输出允许代理以特定、可预测的格式返回数据。无需解析自然语言响应，你将获得类型化的结构化数据。

<Tip>
本页介绍使用 `createAgent` 的代理结构化输出。要直接在模型上使用结构化输出（不使用代理），请参阅[模型 - 结构化输出](/oss/langchain/models#structured-output)。
</Tip>

LangChain 的预构建 ReAct 代理 `createAgent` 自动处理结构化输出。用户设置他们期望的结构化输出模式，当模型生成结构化数据时，它会被捕获、验证并返回到代理状态的 `structuredResponse` 键中。
...
<Note>
    ...
    ```
    如果指定了工具，模型必须同时支持使用工具和结构化输出。
</Note>

结构化响应将返回到代理最终状态的 `structured_response` 键中。
:::
:::js
## 响应格式

控制代理如何返回结构化数据。你可以提供 Zod schema、任何[标准模式 (Standard Schema)](https://standardschema.dev/)-兼容的模式，或 JSON Schema 对象。默认情况下，代理使用工具调用策略，其中输出通过额外的工具调用创建。某些模型支持原生结构化输出，在这种情况下代理将使用该策略。

你可以通过将 `ResponseFormat` 包裹在 `toolStrategy` 或 `providerStrategy` 函数调用中来控制行为：
...
结构化响应将返回到代理最终状态的 `structuredResponse` 键中。
...
某些模型提供商通过其 API 原生支持结构化输出（例如 OpenAI、xAI (Grok)、Gemini、Anthropic (Claude)）。这是可用时最可靠的方法。
...
<ParamField path="schema" required>
    ...
    - **Dataclasses**：带有类型注解的 Python dataclass。返回 dict。
    - **TypedDict**：类型化字典类。返回 dict。
    - **JSON Schema**：包含 JSON schema 规范的字典。返回 dict。
</ParamField>

<ParamField path="strict">
    可选的布尔参数，用于启用严格模式遵循模式。部分提供商支持（例如 [OpenAI](/oss/integrations/chat/openai) 和 [xAI](/oss/integrations/chat/xai)）。默认为 `None`（禁用）。
</ParamField>
...
<ParamField path="schema" required>
    ...
    - **标准模式**：任何实现了[标准模式 (Standard Schema)](https://standardschema.dev/) 规范的模式
...
提供商原生结构化输出具有高可靠性和严格验证，因为模型提供商强制执行模式。在可用时使用它。
...
`tool_message_content` 参数允许你在生成结构化输出时自定义出现在对话历史记录中的消息：
...
<ParamField path="options.handleError">
    选项参数，包含可选的 `handleError` 参数，用于自定义错误处理策略。
...
`toolMessageContent` 参数允许你在生成结构化输出时自定义出现在对话历史记录中的消息：
...
当模型错误地调用了多个结构化输出工具时，代理会在 @[`ToolMessage`] 中提供错误反馈，并提示模型重试：
...
```
如果 `handle_errors` 是字符串，代理将始终使用固定的工具消息提示模型重试：
```
================================= Tool Message =================================
Name: ProductRating
...
如果 `handle_errors` 是异常类型，代理将仅在引发的异常是指定类型时（使用默认错误消息）重试。在所有其他情况下，异常将被抛出。
...
如果 `handle_errors` 是异常元组，代理将仅在引发的异常是其中一种指定类型时（使用默认错误消息）重试。在所有其他情况下，异常将被抛出。
